...
export function MiningHelp({ onClose }: { onClose: () => void }) {
  const sections: { icon: string; title: string; body: string }[] = [
    { icon: '🗡️', title: '開始武器を選ぶ（＝お宝も変わる）', body: 'つるはし⛏️は常時装備（メイン）。それに加えて1種類の武器を選んで潜る。選べる武器は転生で★を使って増やす。お宝は「武器ごと」にあり、その武器を持ち込んだ周回でしか出ない＝色んな武器を使うと色んなお宝が集まる。' },
    { icon: '🐱', title: '猫は自動で掘る', body: '猫は自動でいちばん近い壁へ向かって掘り進む（手動操作なし）。プレイヤーは走行グリッドの解放・転生・お宝集めで成長させる、のんびり放置ゲー。' },
    { icon: '🎁', title: '走行グリッド（その周だけ・手動）', body: '脇の走行グリッドのマスをコイン🪙で手動解放＝一時強化。解放するほど少しずつ高くなる。中身は毎走ランダム、未到達は隠れていて隣を取ると現れる。コインが足りないと灰色で押せない。⏫一括で払えるだけまとめて、🔄でリロール。貫通・範囲は1階層(リング)に1つまで出る。上限まで埋めたら満タン（上限は図鑑が増えると伸びる）。転生でリセット。' },
    { icon: '📒', title: 'お宝図鑑（全100種・武器別）', body: '採掘中にお宝がランダムドロップ（同じお宝を何個でも）。お宝は武器ごとに属し、その武器を持ち込んだ周回でのみ出る（どの武器で壊しても、持ち込み中なら出る）。効果は「武器個別強化（その武器の火力）」と「全体強化（火力/金運/採掘/会心/俊敏/学び/発掘）」の両方。重なるほど1個あたり弱まる（√）。転生しても消えない最後のやり込み。' },
    { icon: '✦', title: 'レアリティ5段階（深いほど良い）', body: 'コモン→アンコモン→レア→エピック→伝説。レアリティが高いほど「解禁される階が深く」「ドロップ率が低く」「効果が大きい」。伝説は地下11階より深く・超低確率。発掘🔎効果でドロップ率は上がる。コツコツ深く潜って図鑑を埋めるやり込み。' },
