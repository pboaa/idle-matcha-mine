...
export function MiningHelp({ onClose }: { onClose: () => void }) {
  const sections: { icon: string; title: string; body: string }[] = [
    ...
    { icon: '🎁', title: '走行グリッド（その周だけ・手動）', body: 'レベルアップごとに「解放権」が1つ増え、脇の走行グリッドのマスを手動で1つ解放＝一時強化。中身は毎走ランダム、未到達は隠れていて隣を取ると現れる。コイン🪙でも解放でき、解放するほど少しずつ高くなる。⏫一括で解放できるだけまとめて、🔄でリロール。貫通・射程は出ない（それは恒久グリッド側）。転生でリセット。' },
    ...
    { icon: '🔄', title: '転生', body: '階・レベル・コイン・走行グリッドはリセット。★残高・累計★・恒久グリッド・解放済み武器は保持。深く潜れるようになるたび転生して恒久強化を進める“周回”が基本のループ。' },
  ];
  return (
