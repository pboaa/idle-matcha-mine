...
/** お宝図鑑：全100種・個数つき。集めた数だけ✕N、未収集は薄く。レアリティで枠色。 */
function DexBook({ entries }: { entries: readonly MineDexEntryVM[] }) {
  return (
    <div className="grid grid-cols-10 gap-0.5">
      {entries.map((e) => (
        <div key={e.id} title={`${e.emoji}${e.name}（${e.rarityLabel}）／ ${e.text}${e.count > 0 ? ` ／ ×${e.count}（重なるほど1個あたり弱まる）` : ' ／ 未入手'}`}
  ...
  return (
    <div className="flex max-h-[88vh] w-[34rem] flex-col gap-3 overflow-y-auto rounded-2xl bg-stone-900 p-4 shadow-2xl ring-1 ring-stone-700">
      <div className="flex items-center justify-between">
        <h2 className="text-base font-bold text-stone-100">📒 お宝図鑑<span className="ml-2 text-[10px] font-normal text-stone-400">採掘でランダム入手（レアリティが高いほど深く・低確率）</span></h2>
      ...
      <div className="rounded-md bg-stone-800/40 p-2 ring-1 ring-stone-700/40">
        <div className="mb-1 text-[10px] text-stone-400">全{p.dex.total}種。枠色＝レアリティ。重なるほど1個あたりの効果は弱まる（√）。レアリティが高いほど深い階で・低確率で出る。</div>
